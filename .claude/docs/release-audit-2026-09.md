...
### 5.3.1 Уточнение 24.09 — идея владельца «уровень надёжности контакта по префиксу ключа» (QQQQQ…)

Идея: публичный ключ (или fingerprint) начинается с N одинаковых символов, чем больше N — тем дороже ключ и
тем «надёжнее» контакт; сеть приоритизирует такие контакты (аналогия — vanity-адреса Bitcoin, Hashcash).

**Вердикт: как сигнал стоимости — да, как свойство ключа — нет.** Это вариант B из §5.3 (grinding ключа), и
к его недостаткам (амортизация: заплатил один раз — спамишь вечно; GPU-асимметрия 50–1000× — «надёжный» =
«у кого есть H100»; детерминизм seed → identity: при восстановлении из фразы grinding повторяется минутами
или часами на телефоне) добавляются три специфичных для Aira:

1. **Контакты не видят identity-ключ.** По §12.6 каждому контакту выдаётся свой pseudonym-ключ (counter от
   seed), два invitation link нелинкуемы. Префикс на identity-ключе никому не виден; префикс на каждом
   pseudonym — grinding на каждый контакт, и общий префикс у всех псевдонимов одного человека **линкует
   их** (ломает §12.6).
2. **Видимый префикс ломает верификацию.** Пользователи сравнивают fingerprint по первым символам; если
   у «надёжных» контактов первые символы одинаковы по построению, злоумышленник получает бесплатную
   похожесть (vanity-адреса onion v3 — известная проблема Tor). Fingerprint должен быть случайным.
3. **Приоритет по цене ключа — это купленный приоритет.** Атакующий с фермой получает высший tier навсегда и
   флудит с приоритетом; честный мобильный пользователь — низший tier. Tor отверг такой дизайн ради
   адаптивного per-connection PoW (hspow).

**Что взять из идеи:** сама мысль «стоимость как сигнал» верна и уже заложена как вариант A §5.3 —
**штамп, а не ключ**: `ContactStamp { epoch, bits, nonce }` = BLAKE3-hashcash над `pseudonym_pk ‖ epoch ‖
bits ‖ nonce` с ведущими нулями; проверяется кем угодно офлайн; **не меняет ключ и seed** (после
восстановления просто пересчитывается в фоне); **истекает** (epoch = неделя) — приоритет нельзя купить
навсегда; считается per-pseudonym и не линкует псевдонимы; мобильный выбирает меньший tier. Штамп
прикладывается к invitation link, `ContactRequest`, `NodeRecord` хопа (§8.7) и даёт tier в очередях:
ContactRequest незнакомцев, `HopSetup` (скидка к базовой сложности вместо `guard_proof`), регистрации
mailbox. Настоящий «уровень надёжности» при этом даёт не вычисление, а **поведение и граф**: контакт
(Tier 1) > контакт контакта (интродукция) > незнакомец со штампом > незнакомец без штампа; для хопов —
наблюдаемый uptime (proof-by-use, §8.7). Место в плане: M22 п.4 («отложено») → поднять в M22 как
`ContactStamp` (1–2 дня в aira-core + поле в link/ContactRequest; UI — бейдж «дорогой контакт» без цифр).
Решение владельца 24.09: **принято** (`owner-decisions.md` C16); M22 п.4 спеки переписан под `ContactStamp`.
...
> Актуальный сводный список и статус ответов (24.09: 16 решений принято) — `audit-2026-09/owner-decisions.md`.
...
## 7. Планы (черновик 07.09 — актуальная версия в спеке)

> **Актуальный план — `spec/18-milestones.md` §16.1 и Milestone 18–28** (правки §8.1–§8.7 и решения владельца 24.09 внесены
> в спеку 24.09.2026). Ниже — исторический черновик от 07.09; расхождения с спекой разрешаются в пользу спеки.
