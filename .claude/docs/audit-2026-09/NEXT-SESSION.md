...
| D | CI/CD и supply chain | `ci-supply-chain-audit.md` | см. §8.4 главного документа: если там ещё заглушка — агент не успел, отчёт частичный, перезапустить по промпту из `phase3-workflow.js` (TOPICS D, пути заменить на Linux/актуальные) |
| E | Остаток спеки (группы, мультидевайс, боты, §6.x, Tauri) | `spec-remainder-audit.md` | ✅ отчёт + §8.5 + `raw/2026-09-23-E-summary.md` |
| F | Community relays (standalone aira-relay, relay в клиенте, N-of-M) | `community-relays.md` | см. §8.6: если заглушка — перезапустить (промпт в `phase3-workflow.js` TOPICS F + граница с G из §8.7: AP-инварианты, роль `hop` по умолчанию, роль `relay`/`mailbox` opt-in) |
...
1. Если §8.4/§8.6 — заглушки: дозапустить D и/или F (по одному агенту, промпты — `phase3-workflow.js`,
   для F добавить границу с G), выжимку в §8.4/§8.6, сводку в `raw/2026-09-23-{D,F}-summary.md`.
2. **Свести решения владельца** из §8.1–§8.7 в один список (≈ 40 пунктов) — в §6.5.4 главного документа
   или отдельным файлом `owner-decisions.md`. Самые срочные (до M18/M19): iroh 1.2; `hide_ip` по умолчанию;
   ссылка без IP; iroh SecretKey per-device; контракт `MessageReceived`; группы/мультидевайс/боты в бете
   отключить; REALITY исключить; transport/* удалить.
3. **Внести правки планов из §8.1–§8.7 в текст M18–M23** `spec/18-milestones.md` и добавить M24a–M24d,
   M25–M28 (нумерация: M24a community relays, M24b Aira Onion v1, M24c мосты/обфускация, M24d mix;
