...
1. **Результат агента wasm** → аудит §6 «Браузерная версия» + детали в «Пересмотр M14» в спеке
   (если сессия оборвалась до записи — см. `raw/` и journal workflow `wf_388eb50e-8c9`,
   `…/subagents/workflows/wf_388eb50e-8c9/journal.jsonl`; salvage транскриптов —
   `scratchpad/salvage/wasm-browser.md`, `wasm.md`).
2. **Коммит документов** (`docs(spec): release audit 2026-09 and milestones M18–M23`) — только
   `.claude/docs/**` и `spec/18-milestones.md`. ⚠️ **Не включать** незакоммиченные
   `Cargo.toml`/`Cargo.lock` владельца (bump ml-dsa rc.4, не собирается — решение: откатить в M18)
   и `habr_article.md` (untracked, правится в M21).
