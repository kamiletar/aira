...
>
> ⚠️ **Фаза 3, продолжение (23.09.2026, темы D–G, аудит §8.4–§8.7).** Добавились: **§8.5** (группы/мультидевайс/Bot API — в бете 0.5 отключить честно; блокеры плана до M19: iroh SecretKey per-device `aira/iroh/secret/<device_index>`, единый контракт `MessageReceived`/`MessageMeta`, `pseudonym_counter` по `device_index`; TTL и `/block` сломаны; реализация — M25 группы v2, M26 мультидевайс v2, M27 Bot API v2, M28 §6.x); **§8.7** (скрытие IP и анти-паразитный форвардинг при участии всех пиров, `.claude/docs/audit-2026-09/onion-antiabuse.md`: M19 п.7 — `NetConfig { hide_ip }` → `clear_ip_transports()` iroh 1.2, дефолт true; M19b п.12 — **invitation link без IP** (`EndpointAddr::new(id).with_relay_url(url)`, не `postcard(ep.addr())`); M20 п.4 — `accept_conn_limit/burst` в iroh-relay не реализованы, RelayMap клиента = свои 1–2 relay; M18 — iroh 1.2.0 (09.09.2026); новые **M24a** community relays (M24a.1 каталог + server-relay, M24a.2 роли relay/mailbox в клиенте; §8.6), **M24b** Aira Onion v1, **M24c** мосты/обфускация через `CustomTransport`, **M24d** mix-профиль; инварианты AP-1…AP-7 обязательны для любого форвардящего кода). **§8.4** (CI на `main` красный с 10.04: 18 advisories + clippy 1.98 из-за плавающего `@stable`; в корне нет `LICENSE-MIT`/`LICENSE-APACHE`/README при «MIT OR Apache-2.0» — до тега v0.4.0; `release.yml` с write-токеном исполняет непроверенный `linuxdeploy`; 0 SHA-пинов, нет `permissions`/`--locked`/Dependabot; M18 п.8 → полный список CI-гвардов, M23 п.4 → SLSA L2 + `SHA256SUMS` + auditable + SBOM); **§8.6** (один бинарь `aira-relay` со встроенным iroh-relay и токеном допуска вместо двух юнитов M20/M21; у iroh один home relay на endpoint → watchdog re-home + 2–3 `RelayRef` в приглашении/контакте до беты; client-relay — только транспорт, opt-in, отдельный процесс; push-URL mailbox = SSRF → `push_allowlist`; `cert_mode = "Reloading"` в 1.2). Сводный список решений владельца — `.claude/docs/audit-2026-09/owner-decisions.md`.
>
> ✅ **Решения владельца 24.09.2026** (детали — `owner-decisions.md`, «Итог опроса»): mailbox v2 — две коробки на пару; протокол v2 без совместимости с 0.3.x; DHT после релиза, discovery = pkarr на своём домене; `Platform::Mobile` удалить; **`hide_ip = true` по умолчанию** (relay-only, direct — per-contact opt-in), invitation link без IP; файлы ≥ 1 MB шифровать per-file ключом в M19a; `transport/*` удалить из main (обфускация → `CustomTransport`, M24c); группы/мультидевайс/Bot API в бете отключить (M25/M26/M27 после беты); Android — preview без APK до подписи; бета = egui + CLI, Tauri после беты до 1.0; Aira Onion (M24b) до 1.0; вторая iroh-идентичность под хоп; relay на mail-сервере — вариант A (nginx stream + SNI passthrough, UDP 7842); подписи — SignPath + minisign (без Apple $99 и Android verification: macOS без notarization, честно в INSTALL.md); quick-xml — ignore со сроком, egui 0.36 в M19b; ветки — только `main` + PR, теги через CI-job `verify`; Linux-сборки в контейнере ubuntu:22.04; CI-гварды в M18, подпись/attestations/SBOM — трек «M22-infra» параллельно M19; каталог relay — один офлайн-ключ, 2-of-3 к 1.0, без поддоменов операторам (IP + pin); `ContactStamp` вместо «префикса ключа» (M22 п.4).
